...
with st.sidebar:
    ...
    include_cups = st.checkbox(
        ...
        value=False,
        key=f"cm_include_cups_{selected_file}",
    ...
    include_hidden = st.checkbox(
        ...
        value=False,
        key=f"cm_include_hidden_{selected_file}",
...
if img_bytes_cm:
    st.image(img_bytes_cm, width="stretch")
...
highlight_player = st.selectbox(
    ...
    index=0, key="cm_lb_highlight_player"
)
...
png_download_button(fig, "leaderboard_central_midfielders.png", key="dl_leaderboard_central_midfielders")
# ----------------- END -----------------
...
st.session_state["cm_selected_player"] = player_name  # <-- critical for downstream defaults
...
def _selected_name() -> str:
    return st.session_state.get("cm_selected_player", player_name)
...
with st.expander("Scatter settings", expanded=False):
    ...
    same_pos_scatter = st.checkbox("Limit pool to current position prefix", value=True, key="cm_sc_pos")
    ...
    try:
        ...
        if x_metric not in pool_sc.columns or y_metric not in pool_sc.columns or colour_metric not in pool_sc.columns:
            ...
            if pool_sc.empty:
                ...
                png_download_button(fig, "scatterplot_central_midfielders.png", key="dl_scatter_central_midfielders")
...
if player_row.empty:
    ...
    if rowA_candidates.empty:
        ...
        if not players_b:
            ...
            pB_label = st.selectbox("Player B (blue)", players_b, index=0, key="cm_radar_pb")
            ...
            if _b_idx is None:
                ...
                if not radar_metrics:
                    ...
                    if pool.empty:
                        ...
                        png_download_button(fig_r, "comparison_radar_central_midfielders.png", key="dl_radar_central_midfielders")
# ----------------- END Radar -----------------
...
with st.expander("Similarity settings", expanded=False):
    ...
    if st.session_state.get(_last_key) != sim_preset:
        st.session_state["cm_sim_leagues"] = preset_vals if preset_vals else default_sel
    ...
    sim_leagues = st.multiselect(
        ...
        default=st.session_state.get("cm_sim_leagues", preset_vals if preset_vals else default_sel),
        key="cm_sim_leagues",
    )
...
if not player_row.empty:
    ...
    if not df_candidates.empty:
        ...
        # (same cm_make_ranking_image renderer used up top — identical style)
        # ---------------------------------------------------------
        ...
        sim_t1 = st.text_input("Title line 1", sim_default_t1, key=f"cm_sim_title1_{selected_file}")
        sim_t2 = st.text_input("Title line 2", sim_default_t2, key=f"cm_sim_title2_{selected_file}")
        sim_t3 = st.text_input("Title line 3", sim_default_t3, key=f"cm_sim_title3_{selected_file}")

        sim_use_custom_footer = st.checkbox(
            "Custom footer text",
            value=False,
            key=f"cm_sim_use_custom_footer_{selected_file}",
            help="Override the default footer description for this image.",
        )
        ...
        if sim_use_custom_footer:
            sim_custom_footer_text = st.text_area(
                ...
                value="",
                key=f"cm_sim_footer_text_{selected_file}",
                help="Each line here will appear as a separate footer line under the graphic.",
            )
        ...
        sim_export_mode = st.selectbox(
            ...
            ["Standard (auto)", "1920×1080 (banner)"],
            index=0,
            key=f"cm_sim_export_mode_{selected_file}",
        )
        ...
        sim_image_theme = st.selectbox(
            ...
            ["Light", "Dark"],
            index=0,
            key=f"cm_sim_img_theme_{selected_file}",
        )

        sim_enable_highlight = st.checkbox(
            "Highlight players in the ranking image",
            value=False,
            key=f"cm_sim_enable_highlight_{selected_file}",
        )
        sim_highlight_names = []
        if sim_enable_highlight:
            ...
            sim_highlight_names = st.multiselect(
                ...
                default=[],
                key=f"cm_sim_highlight_players_{selected_file}",
            )

        sim_img_bytes = cm_make_ranking_image(
            df_show=out,
        ...
        if sim_img_bytes:
            ...
            st.download_button(
                ...
                mime="image/png",
                key=f"cm_sim_download_{selected_file}",
            )
        else:
...
if missing_cf:
    ...
    with st.expander("Club-fit settings", expanded=False):
        ...
        # Keep a canonical "cm_selected_player" around
        st.session_state["cm_selected_player"] = player_name
        sp = st.session_state["cm_selected_player"]
        ...
        if (
            "cm_cf_target_player" not in st.session_state
            or st.session_state["cm_cf_target_player"] not in target_options_cf
            or st.session_state.get("cm_cf_bound_to") != sp
        ):
            st.session_state["cm_cf_target_player"] = sp if sp in target_options_cf else (target_options_cf[0] if target_options_cf else None)
            st.session_state["cm_cf_bound_to"] = sp  # remember which profile we synced from

        # Now render the selectbox (it will show the synced value)
        target_player_cf = st.selectbox(
            "Target player",
            target_options_cf,
            index=target_options_cf.index(st.session_state["cm_cf_target_player"]) if target_options_cf and st.session_state["cm_cf_target_player"] in target_options_cf else 0,
            key="cm_cf_target_player"
        )
    ...
    # -------------------- Compute --------------------
    target_player_val = st.session_state.get("cm_cf_target_player")
