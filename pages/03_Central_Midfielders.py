...
top_n_cm = 10
st.dataframe(df_display.head(top_n_cm), use_container_width=True)
...
# ✅ If photo can't load -> None (blank). NO placeholder.
# ✅ Option to hide/remove player photo (left badge)
# ============================
import io
import re
import unicodedata
...
if player_row.empty:
    ...
    if enable_images:
        # ✅ NEW: compute auto images if enabled (uploads still override)
        ...
        if auto_images:
            ...
            if not hide_player_photo:
                auto_pil_left  = _npimg_to_pil_rgba(_auto_player_photo(player_row))  # may be None (blank)
