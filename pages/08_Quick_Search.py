...
# ==========================================================
# MULTI-POSITION IMPACT FEATURE BLOCK
# - Position-select (or All)
# - Impact/Complete + Raw metric mode
# - Normalise scores vs POSITION group (even when displaying All)
# ==========================================================
...
# ---------- Metric requirements by position ----------
# (These are the metrics used in your formulas. If some datasets miss some,
# you can either add OPTIONAL_METRICS earlier or add safety defaults here.)
POS_METRICS = {
    "CB": [
...
# ---------- Ensure missing metric columns exist (soft safety) ----------
...
# ---------- Percentiles vs POSITION (not league) ----------
...
# ---------- Generic impact builder ----------
def ensure_impact_metrics_for_pos(df: pd.DataFrame, pos_code: str, selected_file: str) -> pd.DataFrame:
    """
    Creates the following columns for that position group (only computed for rows in that group):
      - Sub-scores
      - Base {POS} Score
      - Minutes Factor, Team Context Factor
      - League Factor, Raw Impact No League, Raw Impact Score
      - Impact Score, Impact Score (no league), Complete Score
    """
    df = df.copy()
    ...
    # Minutes factor (you can keep per-league if you want; this is within position group)
    minutes_pct = df.loc[mask].groupby("League")["Minutes played"].rank(pct=True)
    df.loc[mask, "Minutes Factor"] = 0.90 + 0.20 * minutes_pct

    # Team context factor (same idea as your CB: compare team avg to league avg inside position)
    ...
    # Scale impact to 0–100 WITHIN POSITION (critical for “All positions” display)
    ...
    # Complete score (0–100-ish weighted percentile blend; keep as-is)
...
# candidate list for raw metric mode
def raw_metric_candidates(df: pd.DataFrame) -> list[str]:
    ...
    # keep composite columns selectable via composite mode only
    numeric_cols = []
    for c in df.columns:
        if c in bad:
            continue
...
# Composite options (union across positions)
COMPOSITE_OPTIONS = [
    "Impact Score",
...
top_n = st.slider("Top N", 5, 100, 25, 5, key=f"mp_topn_{selected_file}")

# ---------- 5) Build display dataframe ----------
df_pool = df_f.copy()

# optional position filter for pool+display
...
# underlying base column for display (before scaling)
...
# IMPORTANT:
# - If Position = specific: scale 0–100 within that subset
# - If Position = All: scale 0–100 within EACH position group, then combine
if pos_code is None:
...
df_pool = df_pool.dropna(subset=["_MetricForBars"]).sort_values("_MetricForBars", ascending=False).copy()

# ---------- 6) Output table (combined display, position-relative ranking) ----------
cols_show = ["Player","Team","League","Age","Position","PosGroup", rank_col, "_MetricForBars"]
cols_show = [c for c in cols_show if c in df_pool.columns]

st.caption("Sorting uses **_MetricForBars (0–100)** which is normalised vs the player’s position group (and league-factor if toggled).")
st.dataframe(df_pool[cols_show].head(int(top_n)), use_container_width=True)
