...
with st.sidebar:
    ...
    include_cups = st.checkbox(
        ...
        value=False,
        key=f"fb_include_cups_{selected_file}",
    ...
    include_hidden = st.checkbox(
        ...
        value=False,
        key=f"fb_include_hidden_{selected_file}",
...
if img_bytes_fb:
    st.image(img_bytes_fb, width="stretch")
...
# ----------------- ARCHETYPE FILTERS -----------------
ARCHETYPES = {
    "Vertical Build-Up FB": {
        "desc": "Direct, vertical passer — low pass volume, high progressive passing.",
        "conditions": [
            {
    ...
    "Wide Creator": {
        "desc": "High chance creation via crosses and expected assists.",
        "conditions": [
            {
                "xA per 90 Percentile": ("min", 75),
                "Crosses per 90 Percentile": ("min", 75),
            },
        ],
    },
    "Wide Carrier": {
        "desc": "Carries forward — progressive runner and dribbler, low pass volume.",
        "conditions": [
            {
                "Dribbles per 90 Percentile": ("min", 75),
                "Progressive runs per 90 Percentile": ("min", 75),
                "Successful dribbles, % Percentile": ("min", 40),
                "Passes per 90 Percentile": ("max", 40),
            },
        ],
    },
...
with st.sidebar:
    ...
    enable_archetypes = st.checkbox(
        ...
        value=False,
        key=f"fb_enable_archetypes_{selected_file}"
    )

    if st.session_state.get(f"fb_enable_archetypes_{selected_file}", False):
        for arch_name, arch_def in ARCHETYPES.items():
            st.checkbox(
                arch_name,
                value=False,
                key=f"fb_arch_{arch_name}_{selected_file}",
                help=arch_def["desc"],
            )

# Apply the filter to df_f — read everything from session state
enable_archetypes = st.session_state.get(f"fb_enable_archetypes_{selected_file}", False)
selected_archetypes = [
    arch_name for arch_name in ARCHETYPES
    if st.session_state.get(f"fb_arch_{arch_name}_{selected_file}", False)
]
...
with st.expander("Scatter settings", expanded=False):
    ...
    x_default, y_default = "xA per 90", "Defensive duels won, %"
    ...
    same_pos_scatter = st.checkbox("Limit pool to current position prefix", value=True, key="fb_sc_pos")
    ...
    custom_title = st.text_input("Custom title", "xA per 90 vs Defensive duels won, %", key="sc_title")
...
png_download_button(fig, "fb_archetype_map.png", key="dl_featureq_fullbacks")
...
if not player_row.empty:
    ...
    if not df_candidates.empty:
        ...
        # (same fb_make_ranking_image renderer used up top — identical style)
        # ---------------------------------------------------------
        ...
        sim_t1 = st.text_input("Title line 1", sim_default_t1, key=f"fb_sim_title1_{selected_file}")
        sim_t2 = st.text_input("Title line 2", sim_default_t2, key=f"fb_sim_title2_{selected_file}")
        sim_t3 = st.text_input("Title line 3", sim_default_t3, key=f"fb_sim_title3_{selected_file}")

        sim_use_custom_footer = st.checkbox(
            "Custom footer text",
            value=False,
            key=f"fb_sim_use_custom_footer_{selected_file}",
            help="Override the default footer description for this image.",
        )
        ...
        if sim_use_custom_footer:
            sim_custom_footer_text = st.text_area(
                ...
                value="",
                key=f"fb_sim_footer_text_{selected_file}",
                help="Each line here will appear as a separate footer line under the graphic.",
            )
        ...
        sim_export_mode = st.selectbox(
            ...
            ["Standard (auto)", "1920×1080 (banner)"],
            index=0,
            key=f"fb_sim_export_mode_{selected_file}",
        )
        ...
        sim_image_theme = st.selectbox(
            ...
            ["Light", "Dark"],
            index=0,
            key=f"fb_sim_img_theme_{selected_file}",
        )

        sim_enable_highlight = st.checkbox(
            "Highlight players in the ranking image",
            value=False,
            key=f"fb_sim_enable_highlight_{selected_file}",
        )
        sim_highlight_names = []
        if sim_enable_highlight:
            ...
            sim_highlight_names = st.multiselect(
                ...
                default=[],
                key=f"fb_sim_highlight_players_{selected_file}",
            )

        sim_img_bytes = fb_make_ranking_image(
            df_show=out,
        ...
        if sim_img_bytes:
            ...
            st.download_button(
                ...
                mime="image/png",
                key=f"fb_sim_download_{selected_file}",
            )
        else:
