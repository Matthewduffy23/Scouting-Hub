...
# 🔍 Detect WORLDFULL season CSVs from seasons/ subfolder
_seasons_dir = Path(__file__).resolve().parent.parent / "seasons"
if not _seasons_dir.exists():
    _seasons_dir = Path.cwd() / "seasons"

_season_files = sorted(_seasons_dir.glob("*WORLDFULL.csv"), reverse=True)
if not _season_files:
    st.error("No season CSV files found in the seasons/ folder.")
    st.stop()

_season_map = {f.name.replace("WORLDFULL.csv", ""): f for f in _season_files}
_season_labels = list(_season_map.keys())
...
selected_season = st.selectbox("Season", _season_labels, index=0, key="cf_season_select")
selected_file   = selected_season
df = load_df(str(_season_map[selected_season]))
