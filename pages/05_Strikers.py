...
if img_bytes_cf:
    st.image(img_bytes_cf, width="stretch")
    st.download_button(
        "Download PNG (CF)",
        data=img_bytes_cf,
...
def render_pro_layout_strikers(df_view: pd.DataFrame, top_n:int=20):
    ...
    if mv_mode == "Max only":
        mv_max_m = st.slider("Max Market Value (millions)", 0.0, 400.0, 30.0, step=0.5, key="pro_mv_max_m_str")
        mv_max = mv_max_m * 1_000_000
    elif mv_mode == "Range":
        mv_min_m, mv_max_m = st.slider("Market Value Range (millions)", 0.0, 400.0, (0.0, 30.0), step=0.5, key="pro_mv_range_m_str")
...
highlight_player = st.selectbox(
    ...
    index=0, key="stk_lb_highlight_player"
)
...
png_download_button(fig, "leaderboard_strikers.png", key="dl_leaderboard_strikers")
# ----------------- END -----------------
...
st.session_state["stk_selected_player"] = player_name  # <-- critical for downstream defaults
...
def _selected_name() -> str:
    return st.session_state.get("stk_selected_player", player_name)
...
if player_row.empty:
    ...
    pos   = _safe_get(player_row, "Position", "ST")
...
with st.expander("Scatter settings", expanded=False):
    ...
    same_pos_scatter = st.checkbox("Limit pool to current position prefix", value=True, key="stk_sc_pos")
    ...
    try:
        ...
        if x_metric not in pool_sc.columns or y_metric not in pool_sc.columns or colour_metric not in pool_sc.columns:
            ...
            if pool_sc.empty:
                ...
                png_download_button(fig, "scatterplot_strikers.png", key="dl_scatter_strikers")
...
if player_row.empty:
    ...
    if rowA_candidates.empty:
        ...
        if not players_b:
            ...
            pB_label = st.selectbox("Player B (blue)", players_b, index=0, key="stk_radar_pb")
            ...
            if _b_idx is None:
                ...
                if not radar_metrics:
                    ...
                    if pool.empty:
                        ...
                        png_download_button(fig_r, "comparison_radar_strikers.png", key="dl_radar_strikers")
# ----------------- END Radar -----------------
...
if not player_row.empty:
    ...
    if not df_candidates.empty:
        ...
        # (same cf_make_ranking_image renderer used up top — identical style)
        # ---------------------------------------------------------
        ...
        sim_t1 = st.text_input("Title line 1", sim_default_t1, key=f"cf_sim_title1_{selected_file}")
        sim_t2 = st.text_input("Title line 2", sim_default_t2, key=f"cf_sim_title2_{selected_file}")
        sim_t3 = st.text_input("Title line 3", sim_default_t3, key=f"cf_sim_title3_{selected_file}")

        sim_use_custom_footer = st.checkbox(
            "Custom footer text",
            value=False,
            key=f"cf_sim_use_custom_footer_{selected_file}",
            help="Override the default footer description for this image.",
        )
        ...
        if sim_use_custom_footer:
            sim_custom_footer_text = st.text_area(
                ...
                value="",
                key=f"cf_sim_footer_text_{selected_file}",
                help="Each line here will appear as a separate footer line under the graphic.",
            )
        ...
        sim_export_mode = st.selectbox(
            ...
            ["Standard (auto)", "1920×1080 (banner)"],
            index=0,
            key=f"cf_sim_export_mode_{selected_file}",
        )
        ...
        sim_image_theme = st.selectbox(
            ...
            ["Light", "Dark"],
            index=0,
            key=f"cf_sim_img_theme_{selected_file}",
        )

        sim_enable_highlight = st.checkbox(
            "Highlight players in the ranking image",
            value=False,
            key=f"cf_sim_enable_highlight_{selected_file}",
        )
        sim_highlight_names = []
        if sim_enable_highlight:
            ...
            sim_highlight_names = st.multiselect(
                ...
                default=[],
                key=f"cf_sim_highlight_players_{selected_file}",
            )

        sim_img_bytes = cf_make_ranking_image(
            df_show=out,
        ...
        if sim_img_bytes:
            ...
            st.download_button(
                ...
                mime="image/png",
                key=f"cf_sim_download_{selected_file}",
            )
        else:
...
if missing_cf:
    ...
    with st.expander("Club-fit settings", expanded=False):
        ...
        # Keep a canonical "stk_selected_player" around
        st.session_state["stk_selected_player"] = player_name
        sp = st.session_state["stk_selected_player"]
        ...
        if (
            "stk_cf_target_player" not in st.session_state
            or st.session_state["stk_cf_target_player"] not in target_options_cf
            or st.session_state.get("stk_cf_bound_to") != sp
        ):
            st.session_state["stk_cf_target_player"] = sp if sp in target_options_cf else (target_options_cf[0] if target_options_cf else None)
            st.session_state["stk_cf_bound_to"] = sp  # remember which profile we synced from

        # Now render the selectbox (it will show the synced value)
        target_player_cf = st.selectbox(
            "Target player",
            target_options_cf,
            index=target_options_cf.index(st.session_state["stk_cf_target_player"]) if target_options_cf and st.session_state["stk_cf_target_player"] in target_options_cf else 0,
            key="stk_cf_target_player"
        )
    ...
    # -------------------- Compute --------------------
    target_player_val = st.session_state.get("stk_cf_target_player")
