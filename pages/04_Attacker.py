...
with st.sidebar:
    ...
    include_cups = st.checkbox(
        ...
        value=False,
        key=f"att_include_cups_{selected_file}",
    ...
    include_hidden = st.checkbox(
        ...
        value=False,
        key=f"att_include_hidden_{selected_file}",
        help="Lower-tier / obscure leagues hidden by default (Bolivia 1., UAE 1., Argentina 2., etc.).",
    )
    ...
    use_band1 = b1.checkbox("Band 1", value=True,  key=f"att_band1_{selected_file}")
    use_band2 = b2.checkbox("Band 2", value=True,  key=f"att_band2_{selected_file}")
    use_band3 = b3.checkbox("Band 3", value=True,  key=f"att_band3_{selected_file}")

    c4, c5, c6 = st.columns(3)
    use_band4 = c4.checkbox("Band 4", value=True,  key=f"att_band4_{selected_file}")
    use_band5 = c5.checkbox("Band 5", value=True,  key=f"att_band5_{selected_file}")
    use_band6 = c6.checkbox("Band 6", value=True,  key=f"att_band6_{selected_file}")
    ...
    st.session_state[f"att_gbe_bands_{selected_file}"] = selected_bands
...
bands_sel = st.session_state.get(f"att_gbe_bands_{selected_file}", [1, 2, 3, 4, 5, 6])
if bands_sel:
...
# ===================================================================
#  ATT IMPACT FEATURE BLOCK – METRICS + CRESTS + CIES-STYLE IMAGE
...
@st.cache_data(show_spinner=False)
def att_load_remote_png(url: str):
    try:
...
def att_scale_0_100(s: pd.Series, default: float = 50.0) -> pd.Series:
...
def att_pct(m: str) -> str:
    return f"{m} Percentile"


def ensure_att_impact_metrics(df_f: pd.DataFrame, selected_file: str) -> pd.DataFrame:
    """
    Build attacker sub-scores, Impact Score (with & without league),
    ...
    if not has_all_old:
        ...
        df_f["Carrying Score"] = (
            0.40 * df_f[att_pct("Dribbles per 90")] +
            0.20 * df_f[att_pct("Successful dribbles, %")] +
            0.40 * df_f[att_pct("Progressive runs per 90")]
        )

        # Playmaking: progressive passing & entry into final third
        df_f["Playmaking Score"] = (
            0.50 * df_f[att_pct("Progressive passes per 90")] +
            0.25 * df_f[att_pct("Smart passes per 90")] +
            0.25 * df_f[att_pct("Passes to final third per 90")]
        )

        # Chance creation: xA + penalty-area supply
        df_f["Chance Creation Score"] = (
            0.75 * df_f[att_pct("xA per 90")] +
            0.25 * df_f[att_pct("Passes to penalty area per 90")]
        )

        # Goal threat: finishing & volume + box occupation
        df_f["Goal Threat"] = (
            0.40 * df_f[att_pct("xG per 90")] +
            0.30 * df_f[att_pct("Non-penalty goals per 90")] +
            0.15 * df_f[att_pct("Shots per 90")] +
            0.15 * df_f[att_pct("Touches in box per 90")]
        )
        ...
        df_f["Base ATT Score"] = df_f[sub_scores].mean(axis=1)
        ...
        # ----- Team context factor -----
        league_avg = df_f.groupby("League")["Base ATT Score"].transform("mean")
        team_avg   = df_f.groupby(["League", "Team"])["Base ATT Score"].transform("mean")
        ...
        beta_league = float(st.session_state.get(f"att_beta_{selected_file}", 0.40))
        ...
        df_f["Impact Score"]             = att_scale_0_100(df_f["Raw Impact Score"]).astype(float)
        df_f["Impact Score (no league)"] = att_scale_0_100(df_f["Raw Impact No League"]).astype(float)

    # ----- Complete Score (overall attacking profile) -----
    if "Complete Score" not in df_f.columns:
        df_f["Complete Score"] = (
            0.10 * df_f[att_pct("Accurate passes, %")] +
            0.15 * df_f[att_pct("Dribbles per 90")] +
            0.10 * df_f[att_pct("Progressive runs per 90")] +
            0.05 * df_f[att_pct("Passes to final third per 90")] +
            0.20 * df_f[att_pct("xA per 90")] +
            0.10 * df_f[att_pct("Passes to penalty area per 90")] +
            0.10 * df_f[att_pct("Non-penalty goals per 90")] +
            0.20 * df_f[att_pct("xG per 90")]
        )

    return df_f


df_f = ensure_att_impact_metrics(df_f, selected_file)
...
rank_mode = st.radio(
    "Ranking mode (Attackers)",
    ["Composite (Attacker scores)", "Raw metric (any numeric column)"],
    index=0,
    horizontal=True,
    key=f"att_rank_mode_{selected_file}",
)
...
def att_raw_metric_candidates(df: pd.DataFrame):
    bad = {
        "Impact Score", "Impact Score (no league)",
        "Carrying Score", "Playmaking Score", "Chance Creation Score", "Goal Threat",
        "Base ATT Score", "Raw Impact Score", "Raw Impact No League",
...
raw_metric_list = att_raw_metric_candidates(df_f)

use_custom_combo = False
custom_combo_components = []

if rank_mode == "Composite (Attacker scores)":
    use_custom_combo = st.checkbox(
        "Use custom combination of base attacker scores (equal weights)",
        value=False,
        key=f"att_use_custom_combo_{selected_file}",
        help="Combine any of Carrying/Playmaking/Chance Creation/Goal Threat into a single equal-weight score.",
    )
    if use_custom_combo:
        custom_combo_components = st.multiselect(
            "Base scores to include in custom combo (Attackers)",
            BASE_COMPOSITE_COLS,
            default=["Carrying Score", "Chance Creation Score", "Goal Threat"],
            key=f"att_custom_combo_components_{selected_file}",
        )
        rank_label = "Custom Combo"
    else:
        rank_label = st.selectbox(
            "Ranking metric (attacker composite)",
            list(RANK_OPTIONS.keys()),
            index=0,
            key=f"att_rank_metric_{selected_file}",
        )
else:
    default_raw = (
        "xG per 90"
...
if selected_display_league != "All leagues":
    ...
    selected_display_team = st.selectbox(
        "Display team (does not change pool, Attackers)",
        ["All teams"] + teams_in_league,
        index=0,
        key=f"att_display_team_dd_{selected_file}",
    )

display_ls_min, display_ls_max = st.slider(
    "Display league strength range (does not change pool, Attackers)",
    min_value=0,
    max_value=100,
    value=(0, 100),
    step=1,
    key=f"att_display_ls_range_{selected_file}",
)

max_rank_age = st.number_input(
    "Max age in displayed list/image (does not change pool, Attackers)",
    min_value=16, max_value=40, value=23, step=1,
    key=f"att_display_age_{selected_file}",
)

show_age_in_image = st.checkbox(
    "Show age in ranking image (Attackers)",
    value=False,
    key=f"att_show_age_img_{selected_file}",
)

show_league_strength_col = st.checkbox(
    "Show League Strength column in table (Attackers)",
    value=True,
    key=f"att_show_ls_col_{selected_file}",
)

image_theme = st.selectbox(
    "Image theme (Attackers)",
    ["Light", "Dark"],
    index=0,
    key=f"att_img_theme_{selected_file}",
)

enable_highlight_players = st.checkbox(
    "Highlight players in the ranking image (Attackers)",
    value=False,
    key=f"att_enable_highlight_{selected_file}",
)
...
if enable_highlight_players:
    ...
    highlight_player_names = st.multiselect(
        "Players to highlight (Attackers)",
        options=player_opts,
        default=[],
        key=f"att_highlight_players_{selected_file}",
    )
...
if rank_mode == "Composite (Attacker scores)":
    if use_custom_combo:
        ...
        if valid_components:
            ...
            df_pool["Custom Combo Raw"] = df_pool["Base ATT Score"]
...
st.dataframe(df_table.head(top_n_fb), width="stretch")
...
for d in ATT_BADGE_DIRS:
    d.mkdir(exist_ok=True)


def _att_clean_filename(name: str) -> str:
    return re.sub(r"[^a-zA-Z0-9]+", "_", (name or "").lower()).strip("_")


@st.cache_data(show_spinner=False)
def att_load_local_badge(team: str):
    key = _att_clean_filename(team)
    if not key:
        return None
    for folder in ATT_BADGE_DIRS:
...
def att_footer_lines_for_metric(metric_label: str, show_ls: bool):
    ...
    if metric_label == "Impact Score" or metric_label.startswith("Impact Score"):
        return [
            "Impact Score (Attackers): combines Carrying, Playmaking, Chance Creation and Goal Threat.",
    ...
    if metric_label.startswith("Complete Score"):
        return [
            "Complete Score (Attackers): weighted blend of passing, carrying, progression, xG/xA and box threat.",
            f"Displayed 0–100 vs the full selected pool {ls_txt}",
        ]
    if metric_label.startswith("Custom Combo"):
        return [
            "Custom Combo (Attackers): equal-weight blend of chosen base scores (Carrying/Playmaking/Chance Creation/Goal Threat).",
            f"Displayed vs the selected pool {ls_txt}",
        ]
    return [
        f"{metric_label} (Attackers): ranks this metric only.",
        f"Displayed 0–100 vs the full selected pool {ls_txt}",
    ]
...
def att_make_ranking_image(
    ...
    if custom_footer_text:
        ...
        footer_lines = att_footer_lines_for_metric(metric_label, show_ls)
    ...
    if export_mode == "1920×1080 (banner)":
        ...
        for i, (_, row) in enumerate(df_top.iterrows()):
            ...
            ax.text(
                VAL_X, y, att_format_value(v_lab),
    ...
    for i, (_, row) in enumerate(df_top.iterrows()):
        ...
        badge = att_get_team_badge(row)
        if badge is not None:
            z = att_zoom_to_fit(badge, target_px=40)
            ax.add_artist(AnnotationBbox(
                OffsetImage(badge, zoom=z),
                (crest_x, y),
                frameon=False,
        ...
        v_lab = row.get(value_label_col)
        ax.text(VAL_X, y, att_format_value(v_lab),
...
st.subheader("🖼 Exportable CIES-style ranking image – Attackers")

default_t1_att = "TOP ATTACKERS"
default_t2_att = str(metric_label_for_image).upper()
default_t3_att = "PERFORMANCE INDEX  |  Wyscout"

t1_att = st.text_input("Title line 1 (Attackers)", default_t1_att, key=f"att_title1_{selected_file}")
t2_att = st.text_input("Title line 2 (Attackers)", default_t2_att, key=f"att_title2_{selected_file}")
t3_att = st.text_input("Title line 3 (Attackers)", default_t3_att, key=f"att_title3_{selected_file}")

use_custom_footer_att = st.checkbox(
    "Custom footer text (Attackers)",
    value=False,
    key=f"att_use_custom_footer_{selected_file}",
    help="Override the default footer description for this image.",
)

custom_footer_text_att = ""
if use_custom_footer_att:
    custom_footer_text_att = st.text_area(
        "Footer text (multi-line, optional) – Attackers",
        value="",
        key=f"att_footer_text_{selected_file}",
        help="Each line here will appear as a separate footer line under the graphic.",
    )

export_mode_att = st.selectbox(
    "Export format (Attackers)",
    ["Standard (auto)", "1920×1080 (banner)"],
    index=0,
    key=f"att_export_mode_{selected_file}",
)

att_brand_logo_url = "https://image.pitchbook.com/1xOUzrEhnsKrJbNbN8Asf3LND2u1605464042293_200x200"

img_bytes_att = att_make_ranking_image(
    df_show=df_display,
    metric_col=display_metric_col,
    value_label_col=value_label_col,
    metric_label=str(metric_label_for_image),
    title_lines=[t1_att, t2_att, t3_att],
    brand_logo_url=att_brand_logo_url,
    ...
    export_mode=export_mode_att,
    theme=image_theme,
    custom_footer_text=custom_footer_text_att if use_custom_footer_att else None,
)

if img_bytes_att:
    st.image(img_bytes_att, width="stretch")
    st.download_button("Download PNG (Attackers)", data=img_bytes_att,
...
ARCHETYPES = {
    "Semenyo Spec": {
        "desc": "Aerial threat combined with dribbling ability and goal threat.",
        "conditions": [
            {
    ...
    "Moylan Spec": {
        "desc": "Low pass volume, high chance creation via smart passes and xA, with goal threat.",
        "conditions": [
            {
...
with st.sidebar:
    ...
    enable_archetypes = st.checkbox(
        ...
        value=False,
        key=f"att_enable_archetypes_{selected_file}"
    )

    if st.session_state.get(f"att_enable_archetypes_{selected_file}", False):
        for arch_name, arch_def in ARCHETYPES.items():
            st.checkbox(
                arch_name,
                value=False,
                key=f"att_arch_{arch_name}_{selected_file}",
                help=arch_def["desc"],
            )

# Apply the filter to df_f — read everything from session state
enable_archetypes = st.session_state.get(f"att_enable_archetypes_{selected_file}", False)
selected_archetypes = [
    arch_name for arch_name in ARCHETYPES
    if st.session_state.get(f"att_arch_{arch_name}_{selected_file}", False)
]
...
def filtered_view(df_in: pd.DataFrame, *, age_max=None, contract_year=None, value_max=None):
    ...
    if contract_year is not None:
        years = pd.to_datetime(t["Contract expires"], errors="coerce", dayfirst=True).dt.year
...
def render_pro_layout(df_view: pd.DataFrame, top_n:int=20):
    ...
    if team_search_text:
        ...
        if team_terms and "Team" in df_filtered.columns:
            ...
            tmask = pd.Series(False, index=df_filtered.index)
            for t in team_terms:
    ...
    if "Contract expires" in df_filtered.columns:
        ...
        if contract_choice != "Any":
            try:
                ...
                df_filtered["_contract_year"] = pd.to_datetime(df_filtered["Contract expires"], errors="coerce", dayfirst=True).dt.year
    ...
    if foot_vals:
        selected_feet = st.multiselect("Foot", options=foot_vals, default=[], key="pro_foot_filter_att")
        if selected_feet:
    ...
    use_role_filters = st.checkbox("Filter by minimum role score(s)", value=False, key="att_role_filter_toggle")
    if use_role_filters and ROLE_SCORE_COLS_FILTER:
        ...
        for col in ROLE_SCORE_COLS_FILTER:
            ...
            minima[col] = st.slider(f"Min {pretty}", 0, 99, 0, 1, key=f"att_min_{_norm(col)}")
...
highlight_player = st.selectbox(
    ...
    index=0, key="att_lb_highlight_player"
)
...
png_download_button(fig, "leaderboard_attackers.png", key="dl_leaderboard_attackers")
# ----------------- END -----------------
...
st.session_state["att_selected_player"] = player_name  # <-- critical for downstream defaults
...
def _selected_name() -> str:
    return st.session_state.get("att_selected_player", player_name)
...
with st.expander("Scatter settings", expanded=False):
    ...
    same_pos_scatter = st.checkbox("Limit pool to current position prefix", value=True, key="att_sc_pos")
    ...
    try:
        ...
        if x_metric not in pool_sc.columns or y_metric not in pool_sc.columns or colour_metric not in pool_sc.columns:
            ...
            if pool_sc.empty:
                ...
                png_download_button(fig, "scatterplot_attackers.png", key="dl_scatter_attackers")
...
st.markdown("---")
st.header("🧭 Feature Q — ATT Archetype Map")
...
handles = [
    ...
    Line2D([0], [0], marker="s", linestyle="None", color="none",
           markerfacecolor=arch_colors["Final Action"], markersize=16, label="Final Action"),
    Line2D([0], [0], marker="s", linestyle="None", color="none",
...
if player_row.empty:
    ...
    # Player A is the selected player — disambiguate if this name matches multiple rows
    pA = player_name
    rowA_candidates = df[df["Player"] == pA]
    if rowA_candidates.empty:
        st.info("Selected player not found in dataset.")
    else:
        if len(rowA_candidates) > 1:
            def _radar_label_a(r):
                parts = [str(r.get("Team", "")).strip(), str(r.get("League", "")).strip()]
                parts = [p for p in parts if p]
                return f"{pA} — {' / '.join(parts)}" if parts else pA
            _a_options = {}
            for _idx, _r in rowA_candidates.iterrows():
                _lbl = _radar_label_a(_r)
                if _lbl in _a_options:
                    _lbl = f"{_lbl} [{_idx}]"
                _a_options[_lbl] = _idx
            _a_labels = list(_a_options.keys())
            _default_a_idx = 0
            if not player_row.empty and player_row.index[0] in _a_options.values():
                _match = [lbl for lbl, idx in _a_options.items() if idx == player_row.index[0]]
                if _match:
                    _default_a_idx = _a_labels.index(_match[0])
            a_choice = st.selectbox(
                f"Multiple players named '{pA}' — choose Player A (red)",
                _a_labels, index=_default_a_idx, key="radar_pa_disambig",
            )
            rowA = df.loc[_a_options[a_choice]]
        else:
            rowA = rowA_candidates.iloc[0]

        # Player B options using the universal position_filter — exclude only Player A's exact
        # row (by index), so a second player sharing Player A's name stays selectable
        pool_pos = df[df["Position"].astype(str).apply(position_filter)].copy()
        pool_pos = pool_pos.dropna(subset=["Player"])
        pool_pos = pool_pos.drop(index=rowA.name, errors="ignore")

        _name_counts_b = pool_pos["Player"].value_counts()
        def _radar_label_b(r):
            nm = r["Player"]
            if _name_counts_b.get(nm, 0) > 1:
                parts = [str(r.get("Team", "")).strip(), str(r.get("League", "")).strip()]
                parts = [p for p in parts if p]
                return f"{nm} — {' / '.join(parts)}" if parts else nm
            return nm
        _b_options = {}
        for _idx, _r in pool_pos.iterrows():
            _lbl = _radar_label_b(_r)
            if _lbl in _b_options:
                _lbl = f"{_lbl} [{_idx}]"
            _b_options[_lbl] = _idx
        players_b = sorted(_b_options.keys())

        if not players_b:
            st.info("No comparison players available for the current universal position filter.")
        else:
            pB_label = st.selectbox("Player B (blue)", players_b, index=0, key="att_radar_pb")
            ...
            if _b_idx is None:
                ...
                if not radar_metrics:
                    ...
                    if pool.empty:
                        ...
                        png_download_button(fig_r, "comparison_radar_attackers.png", key="dl_radar_attackers")
# ----------------- END Radar -----------------
...
with st.expander("Similarity settings", expanded=False):
    ...
    if st.session_state.get(_last_key) != sim_preset:
        st.session_state["att_sim_leagues"] = preset_vals if preset_vals else default_sel
    ...
    sim_leagues = st.multiselect(
        ...
        default=st.session_state.get("att_sim_leagues", preset_vals if preset_vals else default_sel),
        key="att_sim_leagues",
    )
...
if not player_row.empty:
    ...
    if not df_candidates.empty:
        ...
        st.dataframe(out.head(int(top_n_sim)), width="stretch")

        # ---------------------------------------------------------
        # 🖼 Exportable CIES-style similar players image
        # (same att_make_ranking_image renderer used up top — identical style)
        # ---------------------------------------------------------
        st.subheader("🖼 Exportable CIES-style similar players image")

        sim_default_t1 = f"PLAYERS SIMILAR TO {str(player_name).upper()}"
        sim_default_t2 = "ADJUSTED SIMILARITY"
        sim_default_t3 = "SIMILARITY INDEX  |  Wyscout"

        sim_t1 = st.text_input("Title line 1", sim_default_t1, key=f"att_sim_title1_{selected_file}")
        sim_t2 = st.text_input("Title line 2", sim_default_t2, key=f"att_sim_title2_{selected_file}")
        sim_t3 = st.text_input("Title line 3", sim_default_t3, key=f"att_sim_title3_{selected_file}")

        sim_use_custom_footer = st.checkbox(
            "Custom footer text",
            value=False,
            key=f"att_sim_use_custom_footer_{selected_file}",
            help="Override the default footer description for this image.",
        )

        sim_custom_footer_text = ""
        if sim_use_custom_footer:
            sim_custom_footer_text = st.text_area(
                "Footer text (multi-line, optional)",
                value="",
                key=f"att_sim_footer_text_{selected_file}",
                help="Each line here will appear as a separate footer line under the graphic.",
            )

        sim_export_mode = st.selectbox(
            "Export format",
            ["Standard (auto)", "1920×1080 (banner)"],
            index=0,
            key=f"att_sim_export_mode_{selected_file}",
        )

        sim_image_theme = st.selectbox(
            "Image theme",
            ["Light", "Dark"],
            index=0,
            key=f"att_sim_img_theme_{selected_file}",
        )

        sim_enable_highlight = st.checkbox(
            "Highlight players in the ranking image",
            value=False,
            key=f"att_sim_enable_highlight_{selected_file}",
        )
        sim_highlight_names = []
        if sim_enable_highlight:
            sim_player_opts = sorted(out["Player"].dropna().astype(str).unique().tolist())
            sim_highlight_names = st.multiselect(
                "Players to highlight",
                options=sim_player_opts,
                default=[],
                key=f"att_sim_highlight_players_{selected_file}",
            )

        sim_img_bytes = att_make_ranking_image(
            df_show=out,
            metric_col="Adjusted Similarity",
            value_label_col="Adjusted Similarity",
            metric_label="Adjusted Similarity",
            title_lines=[sim_t1, sim_t2, sim_t3],
            brand_logo_url=att_brand_logo_url,
            show_ls=False,
            show_age=False,
            highlight_players=(sim_highlight_names if sim_enable_highlight else None),
            export_mode=sim_export_mode,
            theme=sim_image_theme,
            custom_footer_text=sim_custom_footer_text if sim_use_custom_footer else None,
        )

        if sim_img_bytes:
            st.image(sim_img_bytes, width="stretch")
            st.download_button(
                "Download PNG",
                data=sim_img_bytes,
                file_name=f"similar_players_{str(player_name).replace(' ', '_')}.png",
                mime="image/png",
                key=f"att_sim_download_{selected_file}",
            )
        else:
...
if missing_cf:
    ...
    with st.expander("Club-fit settings", expanded=False):
        ...
        # Keep a canonical "att_selected_player" around
        st.session_state["att_selected_player"] = player_name
        sp = st.session_state["att_selected_player"]
        ...
        if (
            "att_cf_target_player" not in st.session_state
            or st.session_state["att_cf_target_player"] not in target_options_cf
            or st.session_state.get("att_cf_bound_to") != sp
        ):
            st.session_state["att_cf_target_player"] = sp if sp in target_options_cf else (target_options_cf[0] if target_options_cf else None)
            st.session_state["att_cf_bound_to"] = sp  # remember which profile we synced from

        # Now render the selectbox (it will show the synced value)
        target_player_cf = st.selectbox(
            "Target player",
            target_options_cf,
            index=target_options_cf.index(st.session_state["att_cf_target_player"]) if target_options_cf and st.session_state["att_cf_target_player"] in target_options_cf else 0,
            key="att_cf_target_player"
        )
    ...
    # -------------------- Compute --------------------
    target_player_val = st.session_state.get("att_cf_target_player")
        ...
        if df_candidates_cf.empty:
            ...
            if target_all_rows.empty:
                ...
                if club_profiles_cf.empty:
                    ...
                    st.dataframe(results_cf.head(int(top_n_cf)), width="stretch")
