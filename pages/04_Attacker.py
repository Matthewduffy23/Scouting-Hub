...
_ATT_TWEMOJI_SPECIAL = {
    # none right now – GB uses normal ISO2 "gb"
}

_ATT_SPECIAL_FLAG_URLS = {
    # keep empty for attackers (no custom PNGs needed right now)
}

# Country name overrides → ISO2 codes (lowercase)
_ATT_COUNTRY_OVERRIDES = {
    "england": "gb",
    "scotland": "gb",
    "wales": "gb",
    "northern ireland": "gb",
...
@st.cache_data(show_spinner=False)
def att_load_twemoji_png_by_code(code: str):
    try:
...
@st.cache_data(show_spinner=False)
def att_country_to_iso2_soft(name: str):
    n = _att_norm_country(name)
    if not n:
        return None

    if n in _ATT_COUNTRY_OVERRIDES:
        return _ATT_COUNTRY_OVERRIDES[n]
...
def att_birth_country_flag_image(birth_country):
    if not birth_country:
        return None
    norm = _att_norm_country(birth_country)

    key = _ATT_CC_MAP.get(norm)

    if key is None and norm in _ATT_COUNTRY_OVERRIDES:
        cc = _ATT_COUNTRY_OVERRIDES[norm]
        if cc is None:
            return None
        key = cc

    if key is None:
        iso2 = att_country_to_iso2_soft(birth_country)
        if iso2:
            key = iso2
    ...
    if key in _ATT_TWEMOJI_SPECIAL:
        return att_load_twemoji_png_by_code(_ATT_TWEMOJI_SPECIAL[key])
    if key in _ATT_SPECIAL_FLAG_URLS:
        return att_load_remote_png(_ATT_SPECIAL_FLAG_URLS[key])
    if isinstance(key, str) and len(key) == 2:
        return att_load_twemoji_png_by_code(_att_twemoji_code_from_cc(key))
    return None
