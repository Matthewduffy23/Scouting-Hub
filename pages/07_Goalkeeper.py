# app.py — Advanced Goalkeeper Scouting System (dataset-switch safe)
...
# ----------------- PAGE -----------------
st.set_page_config(page_title="Advanced Goalkeeper Scouting System", layout="wide")
...
def position_filter(pos):
    return str(pos).strip().upper().startswith(CB_PREFIXES)
# -------------------------------------------
...
if rank_mode_gk == "Raw metric (any numeric column)":
    value_label_col_gk = rank_label_gk
else:
...
if display_mv_max is not None:
    df_display_gk = df_display_gk[
        pd.to_numeric(df_display_gk["Market value"], errors="coerce") <= float(display_mv_max)
    ]

if display_mv_min is not None:
    df_display_gk = df_display_gk[
        pd.to_numeric(df_display_gk["Market value"], errors="coerce") >= float(display_mv_min)
    ]
...
# ---------------------------------------------------------
# 6B) FotMob crest fallback (team badge)
# ---------------------------------------------------------

try:
    from team_fotmob_urls import FOTMOB_TEAM_URLS as _GK_FOTMOB_TEAM_URLS
except Exception:
    _GK_FOTMOB_TEAM_URLS = {}

def gk_get_fotmob_url(team: str) -> str:
    return (_GK_FOTMOB_TEAM_URLS.get(team) or "").strip()

def _gk_fotmob_team_id_from_url(team_url: str) -> str:
    m = re.search(r"/teams/(\d+)/", str(team_url or ""))
    return m.group(1) if m else ""

def _gk_fotmob_crest_url(team: str) -> str:
    team_url = gk_get_fotmob_url(team)
    tid = _gk_fotmob_team_id_from_url(team_url)
    return f"https://images.fotmob.com/image_resources/logo/teamlogo/{tid}.png" if tid else ""

@st.cache_data(show_spinner=False)
def gk_load_fotmob_crest(team: str):
    url = _gk_fotmob_crest_url(team)
    if not url:
        return None
    return gk_load_remote_png(url)
...
def gk_zoom_to_fit(img, target_px: int = 28) -> float:
    """
...
def gk_get_team_badge(row: pd.Series):
    ...
    crest = gk_load_fotmob_crest(team)
...
def gk_make_ranking_image(
    ...
    if export_mode == "1920×1080 (banner)":
        ...
        for i, (_, row) in enumerate(df_top.iterrows()):
            ...
            if badge is not None:
                z = gk_zoom_to_fit(badge, target_px=52)
    ...
    for i, (_, row) in enumerate(df_top.iterrows()):
        ...
        badge = gk_get_team_badge(row)
        if badge is not None:
            z = gk_zoom_to_fit(badge, target_px=40)
...
if img_bytes_gk:
    st.image(img_bytes_gk, width="stretch")
...
_POS_COLORS = {
    ...
    "RB":"#FF9A3C","LB":"#FF9A3C","RWB":"#FFD34D","LWB":"#FFD34D",
    "DMF":"#31B56B","LDMF":"#31B56B","RDMF":"#31B56B",
    "CMF":"#5FD37A","LCMF":"#5FD37A","RCMF":"#5FD37A","AMF":"#7FE28A",
    "CF":"#6EA8FF","LW":"#6EA8FF","RW":"#6EA8FF","LWF":"#6EA8FF","RWF":"#6EA8FF",
}
...
def render_pro_layout_gk(df_view: pd.DataFrame, top_n: int = 20):
    ...
    .flagchip{ display:inline-flex; align-items:center; gap:6px; padding:0; }
    .flagchip img{ width:26px; height:18px; border-radius:2px; display:block; }
    .chip{ background:transparent; color:#a6a6a6; border:none; padding:0; font-size:15px; line-height:18px; opacity:.92; }
    ...
    .name{ font-weight:800; font-size:22px; color:#e8ecff; margin-bottom:6px; line-height:1.15; }
    .sub{ color:#a8b3cf; font-size:15px; opacity:.9; }
    .posrow{ margin-top:13.5px; }
    .postext{ font-weight:600; font-size:14.5px; margin-right:11px; }
    .rank{ position:absolute; top:10.5px; right:14px; color:#b7bfe1; font-weight:800; font-size:18px; pointer-events:none; }
    .teamline{ color:#dbe3ff; font-size:14px; font-weight:600; margin-top:6.5px; opacity:.95; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    ...
    .m-title{ color:#e8ecff; font-weight:800; margin:4px 0 10px 0; }
    ...
    .m-badge{ min-width:44px; text-align:center; padding:2px 10px; border-radius:8px; font-weight:800; font-size:18.5px; color:#0b0d12; }
    ...
    age_choice = st.selectbox(
        ...
        index=0,
        key="pro_age_filter_gk",
        label_visibility="visible"
    )
    search_text = st.text_input("Search player(s)", value="", key="pro_player_search_gk").strip()
    team_search_text = st.text_input("Search team(s)", value="", key="pro_team_search_gk").strip()
    ...
    if search_text and "Player" in df_filtered.columns:
        ...
        m = pd.Series(False, index=df_filtered.index)
        for t in terms:
            m |= ser.str.contains(t, na=False)
        df_filtered = df_filtered[m]

    if team_search_text and "Team" in df_filtered.columns:
        terms = [t.strip().lower() for t in team_search_text.split(",") if t.strip()]
        ser = df_filtered["Team"].astype(str).str.lower()
        m = pd.Series(False, index=df_filtered.index)
        for t in terms:
            m |= ser.str.contains(t, na=False)
        df_filtered = df_filtered[m]
    ...
    if "Age" in df_filtered.columns and age_choice != "All":
        try:
            ...
            if age_choice.startswith("U"):
                lim = int(age_choice[1:])
                df_filtered = df_filtered[df_filtered["Age_num"] <= lim]
            elif age_choice.endswith("+"):
                lim = int(age_choice[:-1])
                df_filtered = df_filtered[df_filtered["Age_num"] >= lim]
        except Exception:
            pass
    ...
    GK_ROLE_COLS = ["Shot Stopper GK Score", "Ball Playing GK Score", "Sweeper GK Score"]
    ...
    df_filtered["__sort_val"] = pd.to_numeric(df_filtered.get(sort_by, pd.Series(index=df_filtered.index)), errors="coerce")
    ranked = (
        df_filtered
        .sort_values(["__sort_val", all_col], ascending=[asc, False], na_position="last")
        .drop(columns=["__sort_val"])
        .head(top_n)
    ...
    for i, row in ranked.iterrows():
        ...
        pos = str(row.get("Position","")) or "GK"
        ...
        codes = [c for c in _re.split(r"[,\s/;]+", (pos or "").strip().upper()) if c] or ["GK"]
        if "GK" not in codes:
            codes = ["GK"] + codes
        seen = set()
        codes = [c for c in codes if not (c in seen or seen.add(c))]
        pos_html = "".join(f"<span class='postext' style='color:{_pro_chip_color(c)}'>{c}</span>" for c in codes)

        key_id = f"{_norm(player)}|{_norm(team)}"
        avatar_url = resolve_player_photo(
            player=player, team=team, league=league, key_id=key_id,
...
def clean_attacker_label(s: str) -> str:
    ...
    s = s.replace("Long passes per 90", "Long Passes")
    s = s.replace("Accurate long passes, %", "Long Passing %")
...
if player_row.empty:
    ...
    if vals:
        ...
        st.pyplot(fig_perf, width="stretch")
    else:
...
if player_row.empty:
    ...
    st.pyplot(fig_one, width="stretch")
    buf = BytesIO()
    fig_one.savefig(
        buf,
        format="png",
        ...
        if not player_row.empty:
            for col in ["Height","Height (ft)","Height ft","Height (cm)"]:
                if col in player_row.columns and str(player_row.iloc[0][col]).strip():
                    default_height = _fmt_whole(str(player_row.iloc[0][col]).strip())
                    break
    except Exception:
        pass
...
with st.expander("Scatter settings", expanded=False):
    ...
    x_default, y_default = "Prevented goals per 90", "Exits per 90"
...
def _clean_radar_label(s: str) -> str:
    s = s.replace("Passes per 90", "Passes")
    s = s.replace("Accurate passes, %", "Passing %")
    s = s.replace("Long passes per 90", "Long Passes")
    s = s.replace("Accurate long passes, %", "Long Passing %").replace("Conceded goals per 90", "Goals Conceded")
    s = s.replace("Shots against per 90", "Shots Against").replace("Save rate, %", "Save Rate")
    s = s.replace("Prevented goals per 90", "Goals Prevented").replace("Exits per 90", "Exits")
...
# ============================== FEATURE Q — GK ARCHETYPE SCATTER ==============================
...
st.markdown("---")
st.header("🧭 Feature Q — GK Archetype Map")
...
with st.expander("Scatter settings", expanded=False):
    ...
    df["Minutes played"] = pd.to_numeric(df["Minutes played"], errors="coerce")
    df["Age"] = pd.to_numeric(df["Age"], errors="coerce")
    min_minutes_s, max_minutes_s = st.slider("Minutes", 0, 5000, (500, 5000), key="fq_min_gk")
    min_age_s, max_age_s = st.slider("Age", 14, 45, (16, 40), key="fq_age_gk")
    min_strength_s, max_strength_s = st.slider("League Strength", 0, 101, (0, 101), key="fq_ls_gk")

    # Labels
    show_labels = st.toggle("Show labels", value=True, key="fq_lab_gk")
    label_mode = st.selectbox(
        "Label mode",
        ["Selected player only", "All players", "U23 only", "U21 only", "U18 only"],
        index=0,
        key="fq_label_mode_gk",
    )
    label_size = st.slider("Label size", 8, 20, 12, 1, key="fq_lblsize_gk")

    # Points
    point_size = st.slider("Point size", 24, 300, 225, 2, key="fq_pts_gk")
    ...
    theme = st.radio("Theme", ["Dark", "Light"], index=0, horizontal=True, key="fq_theme_gk")
    ...
    top_gap_px = st.slider("Top gap (px)", 0, 240, 80, 5, key="fq_gap_gk")
    render_exact = st.checkbox("Render exact pixels (PNG)", value=True, key="fq_exact_gk")
...
if pool_sc.empty:
    st.info("No goalkeepers after filtering.")
    st.stop()
...
for (arch, sweeper), grp in pool_sc.groupby(["Archetype", "Sweeper GK"]):
    ax.scatter(
        grp["gk_score"],
        grp["poss_score"],
        ...
        c=arch_colors[arch],
        alpha=point_alpha,
        marker="s" if sweeper else "o",   # square = Sweeper GK, circle = normal
        edgecolors="none",
        linewidth=0,
        zorder=2,
    )
...
handles = [
    ...
    Line2D([0], [0], marker="s", linestyle="None", color="none",
           markerfacecolor=arch_colors["Shot Stopper"], markersize=16, label="Shot Stopper"),
    Line2D([0], [0], marker="s", linestyle="None", color="none",
    ...
    # Sweeper GK header row (no marker)
    Line2D([], [], linestyle="None", color="none", label="Sweeper GK"),
    # No / Yes markers
    Line2D(
        [0], [0],
        marker="o",
...
if render_exact:
    ...
    st.download_button(
        ...
        data=buf.getvalue(),
        file_name=f"feature_q_gk_{uuid.uuid4().hex[:6]}.png",
        mime="image/png",
    )
else:
    st.pyplot(fig)
