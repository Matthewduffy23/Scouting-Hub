...
st.dataframe(df_table.head(top_n), width="stretch")
...
try:
    from team_fotmob_urls import FOTMOB_TEAM_URLS as _CB_FOTMOB_TEAM_URLS
except Exception:
    _CB_FOTMOB_TEAM_URLS = {}

def get_fotmob_url(team: str) -> str:
    return (_CB_FOTMOB_TEAM_URLS.get(team) or "").strip()
...
def zoom_to_fit(img, target_px: int = 28) -> float:
    """
...
def footer_lines_for_metric(metric_label: str, show_ls: bool) -> List[str]:
    ...
    return [
        "%s: ranks this metric only." % metric_label,
        "Displayed 0–100 vs the full selected pool "
        + ("(league strength applied)." if show_ls else "(no league-strength adjustment)."),
    ]
...
def make_ranking_image(
    ...
    buf = io.BytesIO()
    fig.savefig(buf, format="png", dpi=220, facecolor=BG)   # <- no bbox_inches="tight"
    plt.close(fig)
    buf.seek(0)
    return buf.getvalue()
...
# UI lives in the sidebar so it sits alongside the other filters
with st.sidebar:
    st.markdown("---")
    ...
    enable_archetypes = st.checkbox(
        ...
        value=False,
        key=f"cb_enable_archetypes_{selected_file}"
    )

    if st.session_state.get(f"cb_enable_archetypes_{selected_file}", False):
        for arch_name, arch_def in ARCHETYPES.items():
            st.checkbox(
                arch_name,
                value=False,
                key=f"cb_arch_{arch_name}_{selected_file}",
                help=arch_def["desc"],
            )

# Apply the filter to df_f — read everything from session state
enable_archetypes = st.session_state.get(f"cb_enable_archetypes_{selected_file}", False)
selected_archetypes = [
    arch_name for arch_name in ARCHETYPES
    if st.session_state.get(f"cb_arch_{arch_name}_{selected_file}", False)
]
...
def get_fotmob_url(team: str) -> str:
    t = _norm(team)
    for k, v in (FOTMOB_TEAM_URLS or {}).items():
        if _norm(k) == t and str(v).strip():
            return str(v).strip()
    if _external_get_fotmob_url is not None:
        try:
...
COUNTRY_TO_CC = {
    ...
    "western sahara":"eh","réunion":"re","reunion":"re","mayotte":"yt",
    "maroc":"ma","algerie":"dz","tunis":"tn","egypte":"eg","cameroun":"cm","cote d'ivoire":"ci","cote-d-ivoire":"ci",
    "somaliland":"so","ethiopie":"et","gbissau":"gw",
    "brazil":"br","argentina":"ar","uruguay":"uy","chile":"cl","colombia":"co","peru":"pe","ecuador":"ec","paraguay":"py",
    "bolivia":"bo","mexico":"mx","canada":"ca","united states":"us","usa":"us",
    "australia":"au","new zealand":"nz",
    "palestine":"ps","state of palestine":"ps",
...
def render_pro_layout(df_view: pd.DataFrame, top_n: int = 20):
    ...
    if mv_mode == "Max only":
        mv_max_m = st.slider("Max Market Value (millions)", 0.0, 400.0, 30.0, step=0.5, key="pro_mv_max_m")
        mv_max = mv_max_m * 1_000_000
        mv_min = None
    elif mv_mode == "Range":
        mv_min_m, mv_max_m = st.slider("Market Value Range (millions)", 0.0, 400.0, (0.0, 30.0), step=0.5, key="pro_mv_range_m")
    ...
    if "Birth country" in df_filtered.columns:
        ...
        selected_countries = st.multiselect("Birth country", options=country_vals, default=[], key="pro_birth_country_filter")
...
if player_row.empty:
    ...
    buf = BytesIO()
    fig.savefig(buf, format="png", dpi=(150 if enable_images else 130), bbox_inches="tight", facecolor=fig.get_facecolor())
    buf.seek(0)
    st.download_button(
...
if not player_row.empty:
    ...
    if not df_candidates.empty:
        ...
        # (same make_ranking_image renderer used up top — identical style)
        # ---------------------------------------------------------
        ...
        sim_t1 = st.text_input("Title line 1", sim_default_t1, key=f"cb_sim_title1_{selected_file}")
        sim_t2 = st.text_input("Title line 2", sim_default_t2, key=f"cb_sim_title2_{selected_file}")
        sim_t3 = st.text_input("Title line 3", sim_default_t3, key=f"cb_sim_title3_{selected_file}")

        sim_use_custom_footer = st.checkbox(
            "Custom footer text",
            value=False,
            key=f"cb_sim_use_custom_footer_{selected_file}",
            help="Override the default footer description for this image.",
        )
        ...
        if sim_use_custom_footer:
            sim_custom_footer_text = st.text_area(
                "Footer text (multi-line, optional)",
                value="",
                key=f"cb_sim_footer_text_{selected_file}",
                help="Each line here will appear as a separate footer line under the graphic.",
            )
        ...
        sim_export_mode = st.selectbox(
            ...
            ["Standard (auto)", "1920×1080 (banner)"],
            index=0,
            key=f"cb_sim_export_mode_{selected_file}",
        )
        ...
        sim_image_theme = st.selectbox(
            ...
            ["Light", "Dark"],
            index=0,
            key=f"cb_sim_img_theme_{selected_file}",
        )

        sim_enable_highlight = st.checkbox(
            "Highlight players in the ranking image",
            value=False,
            key=f"cb_sim_enable_highlight_{selected_file}",
        )
        sim_highlight_names: List[str] = []
        if sim_enable_highlight:
            ...
            sim_highlight_names = st.multiselect(
                ...
                default=[],
                key=f"cb_sim_highlight_players_{selected_file}",
            )

        sim_img_bytes = make_ranking_image(
            df_show=out,
        ...
        if sim_img_bytes:
            ...
            st.download_button(
                ...
                mime="image/png",
                key=f"cb_sim_download_{selected_file}",
            )
        else:
            st.info("No data to generate image.")
    else:
