...
# ----------------- PRO LAYOUT TAB (tiles) — CENTER BACKS -----------------
...
def render_pro_layout_cb(df_view: pd.DataFrame, top_n: int = 20):
    # CSS (scoped)
    st.markdown("""
    <style>
    ...
    # use already-filtered view (df_f)
    ...
    for i, row in ranked.iterrows():
        ...
        if primary and not position_filter(primary):
            continue  # keep Pro Layout aligned with CB filter
        ...
        bpcb_i = _pro_show99(row.get("Ball Playing CB Score", 0))
        wcb_i  = _pro_show99(row.get("Wide CB Score", 0))
        box_i  = _pro_show99(row.get("Box Defender Score", 0))
        bpcb_txt, wcb_txt, box_txt = _fmt2(bpcb_i), _fmt2(wcb_i), _fmt2(box_i)
          ...
          <div class='pro-card'>
            ...
            <div>
              ...
              <div class='row' style='align-items:center;'>
                <span class='pill' style='background:{_pro_rating_color(bpcb_i)}'>{bpcb_txt}</span>
                <span class='sub'>Ball Playing CB</span>
              </div>
              <div class='row' style='align-items:center;'>
                <span class='pill' style='background:{_pro_rating_color(wcb_i)}'>{wcb_txt}</span>
                <span class='sub'>Wide CB</span>
              </div>
              <div class='row' style='align-items:center;'>
                <span class='pill' style='background:{_pro_rating_color(box_i)}'>{box_txt}</span>
