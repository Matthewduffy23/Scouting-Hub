...
def _split_radar(team_labels, team_pcts, role_labels, role_pcts, who: str):
    ...
    # axes positions tuned for wide split
...
# ---------- role configs (ROLE STYLE = your derived role metrics, same logic; now forced to 5) ----------
def _cfg(role_key: str):
    ...
    if role_key == "cb":
        ...
        def compute(df):
            ...
            out["Aerial Volume"] = _pd.to_numeric(out["Aerial duels per 90"], errors="coerce")
            # (kept computed, even if not shown)
    ...
    if role_key == "cf":
        ...
        label_map = {
            ...
            "Opportunities": "Opportunities",
            "Retention": "Retention",
            "Carrying Outlet": "Carrying Outlet",
        }
        return dict(pos_filter=pos_ok, require_cols=require, compute_metrics=compute, agg_cols=agg_cols,
...
def _sectionB_for_role(role_key: str):
    ...
    # Check required ROLE columns exist (as before)
    ...
    # ROLE metrics computed exactly as before
    pool = cfg["compute_metrics"](pool).copy()

    # TEAM style columns (resolved case-insensitively)
    ...
    # team averages for league (ROLE scope: post-filter, role-position subset)
    agg_role = pool.groupby("Team")[cfg["agg_cols"]].mean().reset_index()

    # team averages for TEAM STYLE columns (same post-filter scope; still grouped by Team)
    agg_team = None
    if team_cols:
        agg_team = pool.groupby("Team")[team_cols].mean().reset_index()

    # target values for ROLE side
    if compare_mode == "Team average":
    ...
    # target values for TEAM side:
    # Even in player mode, "Team Style" should reflect the player's TEAM (target_team).
    target_vals_team = {}
    ...
    # ---------- right side: role team averages table ----------
    with right:
        st.markdown("**Team role averages (post-filter league scope)**")
        sort_col = st.selectbox("Sort by metric", ["Team"] + cfg["agg_cols"], index=0, key=f"secB_sort_{role_key}")
        asc = st.checkbox("Ascending", False, key=f"secB_sort_asc_{role_key}")
        ...
        if agg_team is not None:
            ...
            _download_df_button(
                ...
                key=f"dl_team_agg_{role_key}"
            )

    # ---------- ranks & percentiles: ROLE side ----------
    rows_role = []
    for met in cfg["agg_cols"]:
    ...
    if rank_team_df is not None:
        ...
        _download_df_button(
            rank_team_df,
            f"{cfg['title'].replace(' ','_')}_teamstyle_rank_{label_subject.replace(' ','_')}.csv",
            "⬇️ Download TEAM style ranking (CSV)",
            key=f"dl_team_rank_{role_key}_{label_subject}"
        )
    else:
    ...
    _download_df_button(
        rank_role_df,
        f"{cfg['title'].replace(' ','_')}_rolestyle_rank_{label_subject.replace(' ','_')}.csv",
        "⬇️ Download ROLE style ranking (CSV)",
        key=f"dl_role_rank_{role_key}_{label_subject}"
    )
    ...
    if team_players_used is not None and not team_players_used.empty:
        ...
        _download_df_button(
            ...
            f"{cfg['title'].replace(' ','_')}_{target_team.replace(' ','_')}_players_used.csv",
            "⬇️ Download players used (CSV)",
            key=f"dl_players_used_{role_key}_{target_team}"
        )

    # ---------- split radar (one graph, clear divider, 2x5) ----------
    ...
    if rank_team_df is not None:
        ...
        # fallback placeholders to keep radar callable (but we will show info instead)
        team_labels, team_pcts = [], []

    st.markdown("### 🧭 Team Style vs Role Style (split radar)")
    if team_labels and len(team_labels) == 5 and len(role_labels) == 5:
        ...
        st.download_button(
            ...
            data=buf.getvalue(),
            file_name=f"SectionB_SplitRadar_{cfg['title'].replace(' ','_')}_{label_subject.replace(' ','_')}.png",
            mime="image/png",
            key=f"dl_split_{role_key}_{label_subject}"
        )
        plt.close(fig)
    else:
