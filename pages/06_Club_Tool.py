...
# ======================== SECTION B (v3 — SPLIT RADAR: TEAM STYLE (5) + ROLE STYLE (5)) ========================
# NOTE: You said your ORIGINAL Section B sits ABOVE this in the same page.
# To avoid StreamlitDuplicateElementKey, this version namespaces ALL widget keys with:
#   kbase="secB_SPLIT_v3"
# so it will not collide with the original Section B keys.

st.markdown("---")
st.header("Section B — League Comparison (Team Style + Role Style)")
...
def _download_df_button(df_to_dl: _pd.DataFrame, fname: str, label: str, key: str):
    buf = _io.BytesIO()
    df_to_dl.to_csv(buf, index=False)
    st.download_button(label, buf.getvalue(), file_name=fname, mime="text/csv", key=key)

def _resolve_existing_cols(df_: _pd.DataFrame, pairs):
    """
    pairs: [(pretty_label, column_name), ...]
    Returns: (resolved_pairs, missing_pretty_labels)
    Does case-insensitive matching to tolerate small column-name diffs.
    """
    cols = list(df_.columns)
    lower_map = {str(c).strip().lower(): c for c in cols}
    resolved, missing = [], []
    for pretty, col in pairs:
        key = str(col).strip().lower()
        if col in cols:
            resolved.append((pretty, col))
        elif key in lower_map:
            resolved.append((pretty, lower_map[key]))
        else:
            missing.append(pretty)
    return resolved, missing

# ---------- TEAM STYLE METRICS (your custom 5 per tab) ----------
# NOTE: right-hand strings must match df column names (case-insensitive handled).
TEAM_STYLE_MAP = {
    "cb": [
...
# ---------- split radar (two polar axes in one figure) ----------
...
def _split_radar(team_labels, team_pcts, role_labels, role_pcts, who: str):
    """
    Returns fig with two polar charts (Team Style left, Role Style right) + a vertical divider.
    """
...
# ---------- ROLE STYLE configs (same calculation logic as your v2, but forced to 5 metrics each) ----------
def _cfg(role_key: str):
    ...
    if role_key == "cb":
        ...
        label_map = {c: c for c in agg_cols}
        return dict(pos_filter=pos_ok, require_cols=require, compute_metrics=compute, agg_cols=agg_cols,
                    label_map=label_map, title="Center Backs")
    ...
    if role_key == "fb":
        ...
        label_map = {c: c for c in agg_cols}
        return dict(pos_filter=pos_ok, require_cols=require, compute_metrics=compute, agg_cols=agg_cols,
                    label_map=label_map, title="Fullbacks")
    ...
    if role_key == "cm":
        ...
        label_map = {c: c for c in agg_cols}
        return dict(pos_filter=pos_ok, require_cols=require, compute_metrics=compute, agg_cols=agg_cols,
                    label_map=label_map, title="Central Midfielders")
    ...
    if role_key == "attack":
        ...
        label_map = {c: c for c in agg_cols}
        return dict(pos_filter=pos_ok, require_cols=require, compute_metrics=compute, agg_cols=agg_cols,
                    label_map=label_map, title="Attackers")
    ...
    if role_key == "cf":
        ...
        label_map = {c: c for c in agg_cols}
        return dict(pos_filter=pos_ok, require_cols=require, compute_metrics=compute, agg_cols=agg_cols,
                    label_map=label_map, title="Strikers")

    return None

# ---------- per-role UI/logic ----------
def _sectionB_split_for_role(role_key: str, kbase: str = "secB_SPLIT_v3"):
    cfg = _cfg(role_key)
    if not cfg:
        st.info("Unknown role key.")
        return

    # UNIQUE namespace so it won't clash with your original Section B above
    ...
    # Check required ROLE columns exist
    ...
    # position subset + drop NA required
    ...
    with fl1:
        teams = sorted(league_df["Team"].dropna().astype(str).unique()) if "Team" in league_df.columns else []
    ...
    if "Team" not in league_df.columns or "Player" not in league_df.columns:
        st.info("Missing Team/Player columns.")
        return
    ...
    # ROLE metrics computed (same as before)
    pool = cfg["compute_metrics"](pool).copy()

    # TEAM style columns resolved
    team_pairs_raw = TEAM_STYLE_MAP.get(str(role_key).lower().strip(), [])
    team_pairs, missing_team = _resolve_existing_cols(pool, team_pairs_raw)
    team_cols = [c for _, c in team_pairs]

    if not team_cols:
        st.info("No TEAM style columns found in df for this tab. Check TEAM_STYLE_MAP column names.")
    elif missing_team:
        st.warning(f"Missing TEAM style columns (skipped): {', '.join(missing_team)}")
    ...
    # team averages for ROLE (post-filter role scope)
    agg_role = pool.groupby("Team")[cfg["agg_cols"]].mean().reset_index()

    # team averages for TEAM STYLE (same post-filter scope)
    agg_team = None
    if team_cols:
        agg_team = pool.groupby("Team")[team_cols].mean().reset_index()

    # target values: ROLE
    if compare_mode == "Team average":
        if target_team not in agg_role["Team"].values:
            st.info("Target team has no eligible players in filtered set.")
            return
    ...
    # target values: TEAM STYLE (always the team, even in player mode)
    target_vals_team = {}
    if agg_team is not None and not agg_team.empty and target_team in agg_team["Team"].values:
        target_vals_team = agg_team.set_index("Team").loc[target_team, team_cols].to_dict()
    ...
    with right:
        ...
        if agg_team is not None:
            st.markdown("**Team style averages (post-filter league scope)**")
            st.dataframe(agg_team, use_container_width=True)
            _download_df_button(
                agg_team,
                f"{cfg['title'].replace(' ','_')}_team_style_averages.csv",
                "⬇️ Download team style averages (CSV)",
                key=f"{K}_dl_team_agg"
            )

    # ---------- ranks & percentiles: ROLE ----------
    rows_role = []
    for met in cfg["agg_cols"]:
    ...
    # ---------- ranks & percentiles: TEAM STYLE ----------
    rank_team_df = None
    if agg_team is not None and team_cols and target_vals_team:
        rows_team = []
        for pretty, col in team_pairs:
            temp = agg_team[["Team", col]].copy()
            temp = temp[temp["Team"] != exclude_label].copy()
            val = float(target_vals_team.get(col, _np.nan))
            pseudo = _pd.DataFrame({"Team": [label_subject], col: [val]})
            temp = _pd.concat([temp, pseudo], ignore_index=True)
            temp = temp.drop_duplicates(subset="Team", keep="last")
            ...
            tot = int(temp.shape[0])
            pct = _pct_from_rank(rk, tot)
            rows_team.append((pretty, rk, tot, pct, val))

        rank_team_df = _pd.DataFrame(rows_team, columns=["Metric", "Rank", "Total teams", "Percentile", "Target value"])

    # ---------- summary ----------
    who = f"{label_subject} ({cfg['title']}) vs league team averages"
    st.subheader(f"📈 {who}")

    if rank_team_df is not None:
        st.markdown("### 🧠 Team Style (5 metrics)")
        st.dataframe(rank_team_df, use_container_width=True)
        _download_df_button(
            rank_team_df,
            f"{cfg['title'].replace(' ','_')}_teamstyle_rank_{safe_subject}.csv",
            "⬇️ Download TEAM style ranking (CSV)",
            key=f"{K}_dl_team_rank_{safe_subject}"
        )
    else:
        st.info("Team Style table not available (missing columns or no team style data for selected team).")

    st.markdown("### 🧩 Role Style (5 metrics)")
    ...
    if team_players_used is not None and not team_players_used.empty:
        ...
        st.dataframe(team_players_used[cols_show].sort_values("Minutes played", ascending=False), use_container_width=True)
        _download_df_button(
            team_players_used[cols_show],
            f"{cfg['title'].replace(' ','_')}_{_keyify(target_team)}_players_used.csv",
            "⬇️ Download players used (CSV)",
            key=f"{K}_dl_players_used_{_keyify(target_team)}"
        )
    ...
    if rank_team_df is not None:
        team_labels = rank_team_df["Metric"].astype(str).tolist()
        team_pcts = [int(x) for x in rank_team_df["Percentile"].tolist()]
    else:
        team_labels, team_pcts = [], []

    st.markdown("### 🧭 Team Style vs Role Style (split radar)")
    if team_labels and len(team_labels) == 5 and len(team_pcts) == 5 and len(role_labels) == 5 and len(role_pcts) == 5:
        fig = _split_radar(team_labels, team_pcts, role_labels, role_pcts, who=who)
        st.pyplot(fig, use_container_width=True)
...
with tab_cb:  _sectionB_split_for_role("cb", kbase="secB_SPLIT_v3")
with tab_fb:  _sectionB_split_for_role("fb", kbase="secB_SPLIT_v3")
with tab_cm:  _sectionB_split_for_role("cm", kbase="secB_SPLIT_v3")
with tab_att: _sectionB_split_for_role("attack", kbase="secB_SPLIT_v3")
with tab_st:  _sectionB_split_for_role("cf", kbase="secB_SPLIT_v3")

# ======================== END SECTION B (v3 — SPLIT) ========================
