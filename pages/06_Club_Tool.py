...
#
# Adds filters (DISPLAY ONLY — does NOT change percentile pools / role calc logic):
# ✅ Multi-role filter (ANY/ALL) + min role score
# ✅ League multi-select (display filter)
# ✅ League quality (LEAGUE_STRENGTHS range) (display filter)
# ✅ Age band (display filter)
# ✅ Minutes played band (display filter)
# ✅ Player/Team text search (display filter)
#
# Table shows:
# Rank, Player Photo + Name, Team Crest + Team, League, Age, Minutes, Best role, Score (badge logic + colour)

st.markdown("---")
st.header("📋 Shortlist (Top 20)")

# -------------------- Controls --------------------
c1, c2, c3, c4, c5, c6 = st.columns([1.4, 1.2, 1.2, 1.2, 1.2, 1.8])

with c1:
    group_filter = st.selectbox("Position group", list(POS_GROUPS.keys()), index=0)

# derive a stable role_key for this group (used for role list + scoring)
_group_prefixes = {p.upper() for p in POS_GROUPS[group_filter]}
_group_role_key = ""
for _p in list(_group_prefixes):
    _rk = _role_key_from_pos(_p)
    if _rk:
        _group_role_key = _rk
        break

with c2:
    min_role_score = st.slider("Min role score", 0, 100, 60)

with c3:
    role_match_mode = st.selectbox("Role match", ["ANY selected", "ALL selected"], index=0)

with c4:
    age_band = st.slider("Age band", 14, 45, (16, 32))

with c5:
    mins_band = st.slider("Minutes band", 0, 6000, (0, 6000), step=50)

with c6:
    text_q = st.text_input("Search (player/team)", value="", placeholder="e.g. mbick, colchester...")

# League multi-select (DISPLAY filter only)
all_leagues = sorted(df["League"].dropna().astype(str).unique().tolist()) if "League" in df.columns else []
sel_leagues = st.multiselect("Leagues (display filter)", all_leagues, default=[])

# League quality (DISPLAY filter only) using LEAGUE_STRENGTHS, if available
_ls_map = globals().get("LEAGUE_STRENGTHS", {})
use_league_quality_filter = isinstance(_ls_map, dict) and len(_ls_map) > 0

if use_league_quality_filter:
    q_min_default, q_max_default = 0, 100
    q_range = st.slider("League quality (display filter)", 0, 100, (q_min_default, q_max_default))
else:
    q_range = (0, 100)

# Multi-role selector (based on group role bucket)
role_options = sorted(list(ROLE_BUCKETS.get(_group_role_key, {}).keys()))
sel_roles = st.multiselect("Roles (display filter)", role_options, default=[])

# -------------------- Build shortlist base --------------------
df_short = df.copy()

# position filter (group)
df_short["_pos_tok"] = df_short["Position"].apply(_pos_token)
df_short = df_short[df_short["_pos_tok"].isin(_group_prefixes)].copy()

# display-only league filter
if sel_leagues:
    df_short = df_short[df_short["League"].astype(str).isin([str(x) for x in sel_leagues])].copy()

# display-only age filter
if "Age" in df_short.columns:
    df_short["Age"] = pd.to_numeric(df_short["Age"], errors="coerce")
    df_short = df_short[(df_short["Age"] >= age_band[0]) & (df_short["Age"] <= age_band[1])].copy()

# display-only minutes filter
mins_col = "Minutes played" if "Minutes played" in df_short.columns else None
if mins_col:
    df_short[mins_col] = pd.to_numeric(df_short[mins_col], errors="coerce")
    df_short = df_short[(df_short[mins_col] >= mins_band[0]) & (df_short[mins_col] <= mins_band[1])].copy()

# display-only text search filter
if text_q.strip():
    q = text_q.strip().lower()
    pcol = df_short["Player"].astype(str).str.lower() if "Player" in df_short.columns else ""
    tcol = df_short["Team"].astype(str).str.lower() if "Team" in df_short.columns else ""
    df_short = df_short[pcol.str.contains(q, na=False) | tcol.str.contains(q, na=False)].copy()

# display-only league quality filter
if use_league_quality_filter and "League" in df_short.columns:
    def _lg_q(lg):
        try:
            return float(_ls_map.get(str(lg), 50.0))
        except Exception:
            return 50.0
    df_short["_league_q"] = df_short["League"].astype(str).map(_lg_q)
    df_short = df_short[(df_short["_league_q"] >= q_range[0]) & (df_short["_league_q"] <= q_range[1])].copy()

if df_short.empty:
    st.warning("No players match the display filters.")
    st.stop()
...
# Roles that are display/label-eligible but NOT counted in badge
LABEL_ONLY_ROLES = {
    "box-to-box cm",
    "wide creator fb",
    "wide carrier fb",
...
for idx, ply in df_short.iterrows():
    ...
    # IMPORTANT: keep pools/calculation logic unchanged:
    # reference pool = SAME LEAGUE + SAME POSITION-GROUP for *that player* (not affected by display filters)
    ref_df = df.copy()
    if "League" in ref_df.columns:
        ref_df = ref_df[ref_df["League"].astype(str) == str(ply.get("League", ""))].copy()
    ...
    # role scores for this player
    rs = compute_role_scores(ply, df, role_key, ref_df)
    if not rs:
        continue

    # -------------------- Multi-role filter (DISPLAY ONLY) --------------------
    # If user selected roles, require they meet threshold (ANY/ALL) using role score (not badge score).
    if sel_roles:
        selected_scores = []
        for r in sel_roles:
            if r in rs:
                selected_scores.append(float(rs[r]))
            else:
                selected_scores.append(np.nan)

        if role_match_mode == "ALL selected":
            # must have score for every selected role and each >= min_role_score
            if any(pd.isna(x) for x in selected_scores) or any(x < min_role_score for x in selected_scores):
                continue
        else:
            # ANY selected: at least one selected role exists and >= min_role_score
            ok_any = any((pd.notna(x) and x >= min_role_score) for x in selected_scores)
            if not ok_any:
                continue

    # -------------------- Determine best role + badge score --------------------
    best_role, best_role_score = max(rs.items(), key=lambda kv: kv[1])

    # apply min_role_score when no explicit roles selected (so shortlist still works)
    if (not sel_roles) and float(best_role_score) < float(min_role_score):
        continue

    # compute strengths/weaknesses/styles (used for later extension if needed)
    strengths, weaknesses, styles, _ = compute_strengths_weaknesses_styles(ply, df, role_key, ref_df)

    # badge score excludes Target Man CF + label-only roles
    ...
    top_badge = sorted(filtered_roles_for_badge, key=lambda kv: kv[1], reverse=True)[:1]
    best_val_raw = float(top_badge[0][1]) if top_badge else float(best_role_score)

    # league strength (for badge blending only; same as your pill logic)
    league = str(ply.get("League", ""))
    league_strength = float(_ls_map.get(league, 50.0)) if isinstance(_ls_map, dict) else 50.0

    # use existing controls if present; else fall back
    _use_lw = bool(globals().get("use_league_weighting", True))
    _beta = float(globals().get("BETA_BADGE", 0.40))

    badge_score = (
        (1.0 - _beta) * best_val_raw + _beta * league_strength
        if _use_lw and pd.notna(best_val_raw)
        else best_val_raw
    )
    ...
    if "resolve_player_photo" in globals():
        try:
            photo_url = (resolve_player_photo(str(ply.get("Player","")), str(ply.get("Team","")), league) or "").strip()
        except Exception:
            photo_url = ""
    if "resolve_team_crest" in globals():
        try:
            crest_url = (resolve_team_crest(str(ply.get("Team","")), league) or "").strip()
        except Exception:
            crest_url = ""

    rows.append({
        "Rank": 0,  # fill after sort
        "Player": str(ply.get("Player", "—")),
        "Team": str(ply.get("Team", "—")),
        "League": league,
        "Age": int(ply["Age"]) if pd.notna(ply.get("Age")) else None,
        "Minutes": int(ply[mins_col]) if (mins_col and pd.notna(ply.get(mins_col))) else None,
        "Best role": str(best_role),
        "Score": int(round(float(badge_score))),
        "Photo": photo_url,
        "Crest": crest_url,
    })
...
df_tbl = df_tbl.sort_values("Score", ascending=False).head(20).reset_index(drop=True)
df_tbl["Rank"] = np.arange(1, len(df_tbl) + 1)

# -------------------- Render table (HTML) --------------------
def _img(url, size=32, circle=False):
    if not url:
        return ""
    br = "50%" if circle else "6px"
    return f'<img src="{url}" width="{size}" height="{size}" style="border-radius:{br}; object-fit:cover; vertical-align:middle; margin-right:8px;" />'

html = """
<style>
.shortlist-table { width: 100%; border-collapse: collapse; font-size: 14px; }
.shortlist-table th { text-align: left; padding: 10px 8px; border-bottom: 1px solid rgba(255,255,255,0.12); color: #E5E7EB; }
.shortlist-table td { padding: 10px 8px; border-bottom: 1px solid rgba(255,255,255,0.06); color: #E5E7EB; vertical-align: middle; }
.shortlist-muted { color: #9CA3AF; font-weight: 600; }
.score-pill { display:inline-block; padding: 4px 10px; border-radius: 10px; font-weight: 900; color: white; min-width: 36px; text-align:center; }
</style>
...
for _, r in df_tbl.iterrows():
    col = div_color_tuple(float(r["Score"]))
    rgb = f'rgb({int(col[0]*255)},{int(col[1]*255)},{int(col[2]*255)})'

    player_cell = f'{_img(r["Photo"], 34, circle=True)}<span style="font-weight:900;">{r["Player"]}</span>'
    team_cell = f'{_img(r["Crest"], 28, circle=False)}<span style="font-weight:800;">{r["Team"]}</span>'

    html += f"""
    <tr>
      <td class="shortlist-muted">{int(r["Rank"])}</td>
      <td>{player_cell}</td>
      <td>{team_cell}</td>
      <td>{r["League"]}</td>
      <td>{("" if r["Age"] is None else int(r["Age"]))}</td>
      <td>{("" if r["Minutes"] is None else int(r["Minutes"]))}</td>
      <td style="font-weight:800;">{r["Best role"]}</td>
      <td><span class="score-pill" style="background:{rgb};">{int(r["Score"])}</span></td>
    </tr>
    """
