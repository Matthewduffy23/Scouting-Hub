...
league_df = df[df["League"].astype(str) == included_league].dropna(subset=features).copy()
for c in ["Minutes played","Age","Market value","Goals"]:
...
with flt_col3:
    mv_col = _pd.to_numeric(league_df["Market value"], errors="coerce")
    mv_cap = int(_np.nanmax(mv_col)) if mv_col.notna().any() else 50_000_000
    ...
    if use_millions:
        ...
        mv_min, mv_max = st.slider("Market value (€)", 0, mv_cap, (0, min(mv_cap, 10_000_000)), step=100_000, key="secB_mv_slider_e")
...
df_cf = df_cf[
    ...
    & (df_cf["Market value"].between(mv_min, mv_max))
]
df_cf = df_cf[df_cf["Goals"].between(0, 6000)].copy()  # sanity cap
...
with st.expander("📊 Team CF averages (filtered scope)", expanded=False):
    sort_col = st.selectbox(
        "Sort by", ["Team","Opportunities","Ball Carrying","Aerial Requirement","Passing Volume","Goal Output","Retention"],
...
player_team = str(player_row["Team"].iloc[0])
...
for metric, col in metric_to_teamcol.items():
    ...
    temp = _pd.concat([temp[["Team", col]], pseudo], ignore_index=True)
...
st.subheader(f"📈 Ranks for {target_player} vs other teams’ CF averages {'(incl. own team)' if include_players_team else f'(excl. {player_team})'}")
...
st.download_button(
    ...
    data=buf.getvalue(),
    file_name=f"SectionB_{target_player.replace(' ','_')}.png",
    mime="image/png",
    key=f"secB_dl_{target_player}"
)
plt.close(fig)
